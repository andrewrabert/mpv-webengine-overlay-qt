...
public:
    ...
    void render()
    {
        ...
        // resetOpenGLState() removed in Qt6
...
private slots:
    ...
    void onSynchronize()
    {
        if (!m_renderer && m_mpv) {
            ...
            // Critical settings for overlay technique (Qt6 handles context persistence automatically)
            // setPersistentOpenGLContext() and setPersistentSceneGraph() removed in Qt6
            // Qt6 manages OpenGL context lifecycle automatically
            window()->setColor(Qt::transparent);  // Don't clear - video renders to background
        }
